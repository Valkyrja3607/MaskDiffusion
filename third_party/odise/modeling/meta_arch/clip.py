...
class ClipAdapter(nn.Module):
    ...
    def _encode_image(self, image, is_atten=False):
    ...
    # add
    @torch.no_grad()
    def clip_embed2(self, image, classes):
        image_embed, image_encodings = self._encode_image(self.clip_preprocess(image), is_atten=True)
        print(image_embed.shape, image_encodings.shape)
        text = open_clip.tokenize(classes).to(next(self.parameters()).device)
        text = text[..., : self.max_text_len]
        # text_mask = (text != 0).long()
        text_embed, text_encodings = self._encode_text(text)
        print(text_embed.shape, text_encodings.shape)
        # nums = [len(c.split()) for c in classes]
        text_encodings = text_encodings[:, 1:2, :].mean(dim=1)
        print(text_encodings.shape)
        map = torch.einsum('cf,bfhw->bchw', text_encodings, image_encodings)
        print(map.shape)
        return map

    @torch.no_grad()
    def clip_embed(self, image, classes):
        image = self.clip_preprocess(image)
        print(image.shape)
        x = self.clip.visual(image)
        print(x.shape)
        feat = self.clip.visual.proj(x)
        print(feat.shape)
        text = open_clip.tokenize(classes).to(next(self.parameters()).device)
        text = text[..., : self.max_text_len]
        # text_mask = (text != 0).long()
        ...
        text_encodings = text_encodings[:, 1:2, :].mean(dim=1)
        print(text_encodings.shape)
        map = torch.einsum('cf,bfhw->bchw', text_encodings, feat)
        print(map.shape)
        return map
...
class MaskCLIP(ClipAdapter):
    ...
    def encode_image_with_mask(self, image, mask):
        ...
        # image torch.Size([1, 3, 336, 336]) mask torch.Size([1, 100, 336, 336])
        # clip_image torch.Size([1, 3, 336, 336])
        image = self.clip_preprocess(image)
