...
class CategoryODISE(ODISE):
    ...
    def forward(self, batched_inputs):
        ...
        # extracted_features = self.backbone.feature_extractor({"img": images.tensor})  # add
        ...
        if self.training:
            ...
            for stage, feat in features.items():
                ...
                for idx, f in enumerate(feat):
                    feats[idx][stage] = f[None, :, :, :]
            # print(len(extracted_features))

            for mask_cls_result, mask_pred_result, input_per_image, image_size, feature, mask_embed in zip(
                mask_cls_results, mask_pred_results, batched_inputs, images.image_sizes, feats, mask_embeds
            ):
                ...
                # add
                # processed_results[-1]["feature"] = feature
                # clip feature map
                # mask_embed torch.Size([100, 768])
                # mask_pred_result torch.Size([100, 1024, 2048])
                # mask_pred_result.argmax(0) torch.Size([1024, 2048])
                # clip_map torch.Size([1024, 2048, 768])
                processed_results[-1]["clip_map"] = mask_embed[mask_pred_result.argmax(0)]
            processed_results[-1]["feature"] = features
