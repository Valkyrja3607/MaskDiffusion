...
class LdmExtractor(FeatureExtractor):
    ...
    def unet_forward(self, x, timesteps, context, cond_emb=None):
        ...
        # h = x.type(self.dtype)
        h = x
        # h.shape, emb.shape, context.shape
        # torch.Size([1, 4, 64, 64]) torch.Size([1, 1280]) torch.Size([1, 77, 768])
        for module in unet.input_blocks:
            h = module(h, emb, context)
            hs.append(h)
        h = unet.middle_block(h, emb, context)
        # h torch.Size([1, 1280, 8, 8])

        # add
        # prefix = self.clip.embed_text("car").text_embed
        # B, D = prefix.shape
        # prefix_embed = prefix.expand(B, 77, D)
        # print(x.shape)
        # print(h.shape)
        # x = unet.middle_block[0](h, emb)
        # for layer in unet.middle_block[1].transformer_blocks:
        #     q = layer.to_q(x)

        # print(weight.shape, q.shape, k.shape, v.shape)
        ...
        # unet.out(h) torch.Size([1, 4, 64, 64])
        # ret_features 4
...
class PositionalLinear(nn.Module):
    def __init__(self, in_features, out_features, seq_len=77, bias=True):
        super().__init__()
        # in_features, out_features, seq_len: 768, 1280, 1 or 768, 768, 77
    ...
    def forward(self, x):
        # torch.Size([1, 768])
        x = self.linear(x)
        # torch.Size([1, 1280]) or torch.Size([1, 768])
        x = x.unsqueeze(1) + self.positional_embedding
        # torch.Size([1, 1, 1280]) or torch.Size([1, 77, 768])
        return x
...
class LdmImplicitCaptionerExtractor(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        # prefix.shape torch.Size([1, 768])
        prefix = self.clip.embed_image(image).image_embed
        # prefix_embed.shape torch.Size([1, 77, 768])
        prefix_embed = self.clip_project(prefix)
        # prefix_embed = torch.zeros_like(prefix_embed)

        # # add
        # prefix = self.clip.embed_text(f"a photo of a car").text_embed
        # B, D = prefix.shape
        # prefix_embed = prefix.expand(B, 77, D)

        # cond_inputs.shape torch.Size([1, 77, 768])
