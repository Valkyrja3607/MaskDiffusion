...
#     def single_forward(self, img, caption=[""]):
...
#         features, sims = self.feature_extractor(dict(img=img, caption=caption))
#         # 0 torch.Size([1, 512, 128, 128])
#         # 1 torch.Size([1, 512, 64, 64])
#         # 2 torch.Size([1, 2560, 8, 8])
#         # 3 torch.Size([1, 1920, 16, 16])
#         # 4 torch.Size([1, 960, 32, 32])
#         # 5 torch.Size([1, 640, 64, 64])
#         # 6 torch.Size([1, 512, 64, 64])
#         # 7 torch.Size([1, 512, 128, 128])
#         sl = torch.zeros((1, 77, 64, 64)).cuda()
#         cnt = 0
#         for idx, sim in enumerate(sims):
#             if sim.shape[0] <= 16**2:
#                 cnt += 1
#                 r = int(sim.shape[0]**0.5)
#                 sim = torch.permute(sim.reshape(r, r, 77), (2, 0, 1))[None,:,:,:]
#                 sim = F.interpolate(sim, size=(64, 64), mode="bilinear", align_corners=False).cuda()
#                 sl += sim
#         sl /= cnt
...
#             ), sl
#         else:
#             return self.forward_features(features, input_image_size), sl
...
#     def slide_forward(self, img, caption=[""]):

#         batch_size, _, h_img, w_img = img.shape
#         # output_features = {k: torch.zeros_like(v) for k, v in self.single_forward(img).items()}
#         output_features = {}
#         output_sims = torch.zeros((1, 77, h_img, w_img)).cuda()
#         output_cnt = torch.zeros((1, 77, h_img, w_img)).cuda()
...
#                 crop_features, sims = self.single_forward(crop_img, caption)
...
#                 output_sims[:, :, y1:y2, x1:x2] += F.interpolate(sims, size=(y2-y1, x2-x1), mode="bilinear", align_corners=False).cuda()
#                 output_cnt[:, :, y1:y2, x1:x2] += 1
#         assert all((count_mats[k] == 0).sum() == 0 for k in count_mats)

#         for k in output_features:
#             output_features[k] /= count_mats[k]

#         return output_features, output_sims / output_cnt

#     def forward(self, img, caption=[""]):
#         if (self.training and not self._slide_training) or not self._slide_inference:
#             return self.single_forward(img, caption)
#         else:
#             return self.slide_forward(img, caption)
# ------------------------------------------------------------------------------
# Copyright (c) 2022-2023 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
#
# This work is made available under the Nvidia Source Code License.
